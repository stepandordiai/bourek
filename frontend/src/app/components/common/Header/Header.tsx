						...
						return (
							<React.Fragment key={link.id}>
								{!link.nestedLinks ? (
									<div>
										<Link
											className={`nav__link ${pathname === link.path ? "nav__link--active" : ""}`.trim()}
											href={link.path}
										>
											{t(link.name)}
										</Link>
										...
										<div className="nav__custom-options">
											{link.nestedLinks.map((nestedLink) => {
												return (
													<Link
														key={nestedLink.id}
														className={`nav__link-option ${pathname === `/${nestedLink.id}` ? "nav__link-option--active" : ""}`.trim()}
														href={`/${nestedLink.id}`}
													>
														{t(nestedLink.name)}
													</Link>
								...
								<React.Fragment key={link.id}>
									{!link.nestedLinks ? (
										<li>
											<Link
												onClick={closeMenu}
												className={`burger-menu__nav-link ${pathname === link.path ? "burger-menu__nav-link--active" : ""}`.trim()}
												href={link.path}
											>
												{t(link.name)}
											</Link>
													...
													{link.nestedLinks.map((nestedLink) => {
														return (
															<Link
																key={nestedLink.id}
																onClick={closeMenu}
																className={`burger-menu__dd-link ${pathname === `/${nestedLink.id}` ? "burger-menu__dd-link--active" : ""}`}
																href={`/${nestedLink.id}`}
															>
																{t(nestedLink.name)}
															</Link>
